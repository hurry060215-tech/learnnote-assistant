...
const page = {
  title: "Retry course player",
  page_url: "https://course.example.com/retry",
...
const resources = [{
  url: "https://cdn.example.com/retry/master.m3u8",
  source: "webRequest",
  kind: "hls",
  score: 99,
  label: "HLS",
...
const context = {
  ...
  fetch: async url => {
    ...
    if (value.endsWith("/api/tasks/retry-task")) {
      return {
        json: async () => ({
          task: {
            id: "retry-task",
    ...
    runtime: {
      async sendMessage(message) {
        ...
        if (message.type === "preflight-current-page") {
          return { error: "aggregate unavailable" };
        }
...
context.handleRouteAction("retry-current-page");
await new Promise(resolve => setTimeout(resolve, 0));
await new Promise(resolve => setTimeout(resolve, 0));

assert.ok(calls.collect >= 2);
assert.equal(calls.preflight, 1);
assert.ok(calls.start, "expected retry route action to start a new current-page video task");
