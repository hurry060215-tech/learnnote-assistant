...
assert.doesNotMatch(sidepanelHtml, /问这节课/);
...
assert.match(chaoxingProfileHtml, /平台线索 · 学习通\/超星/);
...
const genericPlatformSignalHtml = context.platformSignalHtml({
  ...
  selected_resource: {
    url: "https://learn.example.com/api/playurl",
    kind: "unknown",
    source: "webRequest",
    request_type: "xmlhttprequest",
    method: "POST",
    request_body: { content: "lesson=1", type: "form" },
    request_headers: { Referer: "https://learn.example.com/course/player", Origin: "https://learn.example.com" },
    frame_url: "https://learn.example.com/player-frame"
  },
  direct_extraction: {
    boundary: "download_forbidden",
    selected_candidate: { has_replay_body: true },
    browser_context: { cookie_domain_count: 1, cookie_count: 2, active_source_type: "blob" }
  },
  page_preflight_report_path: "D:/Projects/learnnote-assistant/data/tasks/generic/page_preflight_report.json"
});
assert.match(genericPlatformSignalHtml, /平台线索 · learn\.example\.com/);
assert.match(genericPlatformSignalHtml, /播放 API/);
assert.match(genericPlatformSignalHtml, /POST\/body/);
assert.match(genericPlatformSignalHtml, /Origin, Referer/);
...
assert.match(reviewWorkbenchHtml, /data-open-workbench="side-review-workbench"/);
...
assert.equal(elements.get("#studyModeButton").classList.contains("active"), true);
assert.equal(elements.get("#diagnosticModeButton").classList.contains("active"), false);
context.setPanelMode("diagnostics");
assert.equal(elements.get("#resourcePanel").hidden, false);
assert.equal(elements.get("#studyModeButton").classList.contains("active"), false);
...
assert.match(elements.get("#currentStudyCard").className, /study-mode/);
assert.match(elements.get("#currentStudyCard").innerHTML, /发现当前视频直取候选/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-brief/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /学习流总览/);
assert.match(elements.get("#currentStudyCard").innerHTML, /evidence-steps/);
assert.match(elements.get("#currentStudyCard").innerHTML, /发现播放器/);
assert.match(elements.get("#currentStudyCard").innerHTML, /找到视频源/);
assert.match(elements.get("#currentStudyCard").innerHTML, /验证可下载/);
assert.match(elements.get("#currentStudyCard").innerHTML, /生成画面切片/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-adapter-ladder/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /通用适配链/);
assert.match(elements.get("#currentStudyCard").innerHTML, /data-route-action="preflight"/);
assert.match(elements.get("#currentStudyCard").innerHTML, /待预检/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-route/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-slice-plan/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-note-preview/);
...
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /https:\/\/cdn\.example\.com\/live\/master\.m3u8/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /data-workbench-copy="url"/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /data-workbench-copy="report"/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /data-workbench-copy="audit"/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /直取审计门/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-steps/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-facts/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-footer/);
...
context.setPanelMode("diagnostics");
assert.match(elements.get("#currentStudyCard").className, /diagnostic-mode/);
...
assert.match(elements.get("#playbackReadiness").innerHTML, /playback-readiness-actions/);
assert.match(elements.get("#playbackReadiness").innerHTML, /data-route-action="preflight"/);
assert.match(elements.get("#playbackReadiness").innerHTML, /data-route-action="summarize"/);
assert.match(elements.get("#playbackReadiness").innerHTML, /data-route-action="download"/);
...
vm.runInContext("els.noteStyle.value = 'concept'; applyLearningGoalDefaults();", context);
assert.equal(elements.get("#noteTemplate").value, "visual-handout");
assert.equal(elements.get("#summaryDepth").value, "deep");
vm.runInContext("els.noteStyle.value = 'concise'; applyLearningGoalDefaults();", context);
assert.equal(elements.get("#noteTemplate").value, "timeline");
assert.equal(elements.get("#summaryDepth").value, "brief");
vm.runInContext("els.noteStyle.value = 'exam'; applyLearningGoalDefaults();", context);
...
assert.match(auditReport, /通用适配链/);
assert.match(auditReport, /播放器入口/);
assert.match(auditReport, /媒体落点/);
...
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-audit-gate/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /直取审计门/);
...
assert.match(elements.get("#currentStudyCard").innerHTML, /生成学习笔记/);
assert.match(elements.get("#currentStudyCard").innerHTML, /发现播放器/);
assert.match(elements.get("#currentStudyCard").innerHTML, /找到视频源/);
assert.match(elements.get("#currentStudyCard").innerHTML, /验证可下载/);
assert.match(elements.get("#currentStudyCard").innerHTML, /生成画面切片/);
...
assert.match(elements.get("#playbackReadiness").innerHTML, /data-route-action="redetect"/);
assert.match(elements.get("#playbackReadiness").innerHTML, /data-route-action="local"/);
assert.match(elements.get("#playbackReadiness").innerHTML, /data-route-action="text"/);
assert.doesNotMatch(elements.get("#playbackReadiness").innerHTML, /data-route-action="summarize"/);
...
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-audit-gate/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /失败边界/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /转本地入口/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-steps/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-facts/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-note-preview/);
assert.doesNotMatch(elements.get("#currentStudyCard").innerHTML, /workbench-local-fallback/);
