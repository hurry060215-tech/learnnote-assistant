...
const context = {
  ...
  fetch: async url => {
    ...
    if (value.endsWith("/api/tasks/task-from-page-report")) {
      return {
        json: async () => ({
          task: {
            id: "task-from-page-report",
            status: "success",
            phase: "completed",
            progress: 100,
            message: "done",
    ...
    if (value.endsWith("/api/tasks/task-from-page-report/transcript")) {
      return { json: async () => ({ segments: [] }) };
    }
    if (value.endsWith("/api/tasks/task-from-page-report/note")) {
    ...
    runtime: {
      async sendMessage(message) {
        ...
        if (message.type === "start-current-task") {
          calls.start = message;
          return { task_id: "task-from-page-report" };
        }
...
assert.equal(calls.start.pagePreflightReport.ready, true);
assert.equal(calls.start.pagePreflightReport.selected_url, "https://cdn.example.com/live/master.m3u8");
assert.equal(calls.start.pagePreflightReport.candidates[0].preflight.code, "download_forbidden");
