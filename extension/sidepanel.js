...
function currentPageAuditReport() {
  ...
  const candidateCount = candidateResources().length;
  const downloadableCount = directExtractionCandidates().length;
...
function visualWindowText() {
  ...
  const visual = readVisualSliceOptions();
  return `约 ${fmt(visual.frame_interval * visual.grid_columns * visual.grid_rows)} / 窗`;
}
...
function directExtractionCandidates() {
  return candidateResources().filter(isDirectExtractionCandidate);
}

function playbackMatchedCandidates() {
...
function loadDiagnosticView() {
  try {
    return {
      size: window.localStorage?.getItem("learnnote.diagnosticSize") === "large" ? "large" : "standard",
      detail: window.localStorage?.getItem("learnnote.diagnosticDetail") === "full" ? "full" : "essential"
    };
  } catch {
...
function saveDiagnosticView() {
  try {
    window.localStorage?.setItem("learnnote.diagnosticSize", diagnosticView.size);
    window.localStorage?.setItem("learnnote.diagnosticDetail", diagnosticView.detail);
  } catch {
    // Preferences are optional in restricted extension previews.
  }
}
...
function diagnosticUserDetail(task) {
  const messages = {
    ...
    no_media_found: "页面暂未暴露可下载的视频资源。请先播放几秒，再重新检测。",
...
function diagnosticSummaryPanel(task) {
  ...
  const checks = [
    ...
    ["画面", windows ? `${windows} 窗口` : task?.options?.visual_understanding === false ? "已关闭" : "未生成", Boolean(windows) || task?.options?.visual_understanding === false],
  ...
  return `<section class="diagnostic-summary-panel ${failed ? "error" : running ? "running" : success ? "success" : "pending"}" data-diagnostic-scale="${escapeHtml(diagnosticView.size)}">
    <header><div><span>${failed ? "需要处理" : running ? "处理中" : success ? "状态正常" : "待检查"}</span><strong>${escapeHtml(title)}</strong><p>${escapeHtml(detail)}</p></div></header>
...
function resourceAttemptQueueHtml(limit = 4) {
  ...
  return `<section class="resource-attempt-queue" aria-label="候选下载队列">
    <div class="resource-attempt-head">
      <strong>下载队列</strong>
      <span>${directExtractionCandidates().length}/${candidateCount} 可直取</span>
    </div>
...
function playbackReadinessState() {
  ...
  const subtitles = page?.browser_subtitles || [];
  const downloadable = directExtractionCandidates();
...
function playbackReadinessCopy(state) {
  ...
  const matched = playbackMatchedCandidates().length;
...
function renderPlaybackReadiness() {
  ...
  const subtitles = page?.browser_subtitles || [];
  const downloadable = directExtractionCandidates();
  const matched = playbackMatchedCandidates();
...
function workbenchRunModeItems(state) {
  ...
  const mediaName = taskMediaDisplayName(currentTask);
  return [
    {
    ...
    },
    {
      ...
      label: "只下载",
      title: canContinue ? `${mediaName} 已落地` : canDirectTask ? "先把视频拉到本地" : "等待媒体 URL",
    ...
    },
    {
      ...
      label: "续跑切片",
      title: canContinue ? `从 ${mediaName} 继续` : blocked ? "改走本地视频" : "等待本地媒体",
      detail: canContinue
        ? "复用已下载视频进入转写、抽帧、视觉窗口和图文总结；不会录制页面。"
        : blocked
          ? "当前页不可直取时，上传本地视频进入同一套切片总结管线。"
          : `只下载成功后，可从本地 ${mediaName} 继续切片总结。`,
...
function resourceHint() {
  ...
  const playbackMatched = playbackMatchedCandidates().length > 0;
...
function routeSummaryMetrics() {
  ...
  const candidateCount = candidateResources().length;
  return [
    { label: "候选", value: `${downloadableCount}/${candidateCount}` },
  ...
  return [
    {
      ...
      label: "资源证据",
      value: selected?.kind || `${downloadableCount}/${candidateCount}`,
...
function pipelineAuditItems(task) {
  ...
  const items = [
    ...
    },
    {
      ...
      value: isPageText ? "文本路线" : hasMedia ? mediaName : task?.error_code || "待下载",
...
function directFailureChoiceItems(task) {
  ...
  if (canContinueFromDownloadedMedia(task)) {
    ...
    return [{
      ...
      action: task.id || "",
      label: "继续切片总结",
      detail: `已下载 ${mediaName}，复用本地视频继续转写、切片和图文总结。`
    }];
  }
...
function directExtractionEvidenceItems(task) {
  ...
  return [
    ...
    },
    {
      ...
      label: "媒体落地",
      value: direct.media_landed ? `已落地 ${mediaName}` : "未落地",
...
function taskNextActionsHtml(task) {
  ...
  const controls = actions.map(action => {
    ...
    if (action.intent === "export" && action.target) {
      return `<button type="button" class="next-action" data-export="${escapeHtml(action.target)}">${content}</button>`;
    }
...
function taskOverview(task) {
  ...
  return `<section class="task-overview status-${statusClass}">
    ...
    ${downloadOnly ? `<div class="task-overview-callout">
      ...
      <span>这个任务按“下载到本地”运行，未进入转写、切片和总结；可导出 ${escapeHtml(mediaName)}，或直接复用这个本地视频生成完整笔记。</span>
    </div>` : ""}
...
function bindTaskOverviewActions() {
  document.querySelectorAll("button[data-diagnostic-size]").forEach(button => {
    button.onclick = () => {
  ...
  document.querySelectorAll("button[data-diagnostic-detail]").forEach(button => {
    button.onclick = () => {
...
function noteReviewWorkbench(markdown, task) {
  ...
  const advanced = [
    ...
    hasDiagnostics ? `<button type="button" data-export="diagnostics">导出诊断</button>` : "",
    hasAudit ? `<button type="button" data-export="audit">导出审计</button>` : "",
    task.resource_inventory_path ? `<button type="button" data-export="resource-inventory">候选证据</button>` : "",
    task.page_preflight_report_path ? `<button type="button" data-export="page-preflight-report">预检报告</button>` : ""
  ].filter(Boolean);
...
function visualStudyCorrelationHtml(task, transcript = null) {
  ...
  const rows = shown.map((window, index) => {
    ...
    const points = visualWindowSummaryItems(window, transcript).slice(0, 2);
    const hasQuestion = Boolean(cues.length || window.transcript_excerpt || (window.frame_timestamps || []).length);
    const cueText = cues.length
      ? `${cues.length} 段 · ${fmt(cues[0].start)} 起`
      : window.transcript_excerpt
        ? "有摘录"
        : "待补字幕";
    const summaryText = points.length
      ? points[0]
      : "等待局部总结";
    return `<article class="${escapeHtml(evidence.state)}" data-visual-window="${escapeHtml(id)}">
      ...
      <div>
        ...
        <small>${escapeHtml(cues[0]?.text ? cues[0].text.slice(0, 86) : window.transcript_excerpt || "切到字幕页核对完整上下文")}</small>
      </div>
      <div>
        <span>局部总结</span>
        <strong>${escapeHtml(summaryText.length > 74 ? `${summaryText.slice(0, 74).trim()}...` : summaryText)}</strong>
        <small>${points[1] ? escapeHtml(points[1].slice(0, 96)) : escapeHtml(evidence.detail)}</small>
      </div>
      <div>
        <span>复习动作</span>
        <strong>${hasQuestion ? "可自测" : "先看片"}</strong>
        <small>${hasQuestion ? "按画面解释字幕中的标题、公式、代码或操作状态" : "先描述截图再补字幕"}</small>
      </div>
      <nav>
        <button type="button" data-media-seek-time="${seekTimeValue(window.start)}">回看</button>
  ...
  return `<section class="visual-review-path" aria-label="切片复核路线">
    ...
    <div class="visual-review-path-list">
      ${shown.map(item => `<article class="${escapeHtml(item.evidence.state)} priority-${item.priority}">
        ...
        <p>${escapeHtml(String(item.summary || item.reason).replace(/\s+/g, " ").slice(0, 120))}</p>
        <nav>
...
function visualStudyNavigatorHtml(task, transcript = null) {
  ...
  return `<section class="side-visual-study-navigator" aria-label="视觉窗口学习队列">
    <header>
      <span>复习队列</span>
...
function renderResult() {
  ...
  if (selectedTab === "diagnostics") {
    ...
    els.result.innerHTML = `
      ${diagnosticSummaryPanel(currentTask)}
