...
- Page-network hook discovery for mp4/HLS/DASH/subtitle URLs embedded in fetch/XHR text responses. The hook also records Blob object URL and MediaSource source mappings when the page builds playback from accessible media responses; it does not record playback or inspect binary video payloads.
...
- `blob:` URLs without an underlying `.m3u8`, `.mpd`, video file request, or recoverable fetch/XHR Blob/ArrayBuffer/MediaSource source mapping are reported as `drm_or_encrypted`.
