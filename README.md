...
| 等完整任务结束才可阅读 | 字幕完成即开放草稿，画面证据随后渐进补充 |
...
- 批量抽帧、重复帧缓存和受控视觉并发减少长视频等待与重复额度消耗。
- UTF-8、UTF-16、GB18030、Shift-JIS 等文本经过严格解码；高置信乱码会被隔离而不是进入笔记。

### 先读起来，再逐步补全

- 字幕完成后立即生成第一版可阅读大纲，不必等待全部视觉分析。
- 任务进度通过可重连事件流实时更新，断线时自动回退轮询。
- 正式笔记提供章节级证据状态、视频时间戳和统一 `SourceEvidence` 出处。
- 崩溃或重试时复用已下载媒体、有效帧和视觉批次缓存。
...
- 支持 Markdown、可编辑 Word、打印级 PDF、字幕、诊断报告、媒体文件和完整学习资料包导出。
- 任务诊断会说明视频来源、下载路径、字幕来源、模型降级和修复建议。

### 本地资料库与学习闭环

- PDF、Markdown、HTML、TXT 和本地视频都可进入同一资料库，保留页码、段落或时间戳锚点。
- 资料库搜索只返回有界摘要，不暴露本机文件路径或整份私有内容。
- 从真实证据生成记忆卡片，使用本地 FSRS 调度复习、错题和今日计划。
- 评论与弹幕属于默认关闭的独立“社区观点”层，不会混入课程主证据。
- 无官方云、无强制账号、无产品遥测；材料、卡片和社区记录都提供本地硬删除入口。
...
1. Chrome 用户优先安装 [Chrome Web Store 正式版](https://chromewebstore.google.com/detail/learnnote-%E5%BD%93%E5%89%8D%E8%A7%86%E9%A2%91%E5%8A%A9%E6%89%8B/mncdchpkpikhacmkbanedpppcddapppe)，后续版本由浏览器自动更新。
2. 打开 LearnNote 客户端，在 **设置 → 浏览器扩展** 检查连接状态。
3. 回到视频页，刷新一次页面，再打开 LearnNote 扩展侧栏。

需要离线安装或调试时，才使用 Release 中的扩展 ZIP：解压后在 `chrome://extensions` 或 `edge://extensions` 开启开发者模式，选择 **加载已解压的扩展程序**。离线包不会自动跟随 Chrome Web Store 更新。
