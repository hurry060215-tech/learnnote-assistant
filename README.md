...
> **客户端 v0.2.10 / 扩展 v0.2.10：** 字幕读取与客户端连接并行，减少重复查询，支持原文预览与搜索。商店版本可能滞后于 GitHub；真实 B 站登录态的提取耗时仍待实测，不保证秒提取。
...
| 没有配置文字模型         | 在下载、转写前提示配置，也可改选“仅提取字幕”     | 原文材料，**不是 AI 总结**               |
...
本地转写采用 faster-whisper，是可选能力。有字幕或使用远程转写时，无需下载本地模型。需要离线转写时，在 **设置 → 字幕与转写 → 准备本地语音模型** 中检查状态并按需下载；速度与视频时长、模型大小、CPU/GPU 和硬件环境有关。画面 OCR 用于抽帧文字提取，**完整画面字幕 OCR 尚未作为自动回退流程提供**。

**保存字幕原文**保留逐字内容和时间点；**生成总结笔记**会调用文字模型提炼要点、大纲与结论，二者在创建时明确区分。模型输出检查失败时保留原文，并提供直接重试总结的入口，不必重新添加视频。

浏览器扩展可以搜索已取得的原文、按停顿和段落长度展开分组，并点击字幕回到原视频；这些分组不是 AI 生成的主题章节。B 站字幕笔记即使没有下载视频，也可在来源区按需打开官方外链播放器；在线观看视频仍需要网络，并受平台播放权限限制。

侧栏的 **我的** 打开个人主页，查看内容数量、本机 Token 用量与复习入口。用量只记录服务商实际返回的数据，不代表账号余额；未返回用量单独标注。复习使用 FSRS，根据评分与历史记忆表现调整间隔，并在评分前显示预计下次复习时间。

在 **设置 → 外观** 中选择“清晰紧凑”或“大字加粗”，也可以分别调整字体、字号、字重、行距和正文宽度。旧的自定义外观会保留；侧栏的 **复习** 先展示今日计划，再进入卡片练习。

未发送的助手问题按笔记保存为本机草稿，刷新或失败后可以接着写。重新生成前保留上一份生成稿，在“历史版本”中预览、复制或下载，个人修改独立保存。同步暂时中断时，当前内容与编辑输入会保留，并提供重试入口。
