...
| Durable mixed scheduler | `python scripts/scheduler-reliability.py` | Five mixed light/heavy local intents, duplicate enqueue and journal reopen | None | Callbacks do not overlap, duplicate enqueue reuses the same future, all journal states reopen as `done`, and no task content is persisted |
| Complete local task | `python scripts/full-local-task-reliability.py --duration-seconds 3600` | Full local 60-minute media/transcript/visual/note pipeline using synthetic media, synthetic timed subtitles, and a deterministic offline summary fixture | None | A 60-minute task reaches `success`, writes note/transcript/visual/resource artifacts, and makes zero remote calls; a real missing-model task remains an explicit failed/retryable outcome |
