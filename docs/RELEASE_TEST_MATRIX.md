...
| Complete local task | `python scripts/full-local-task-reliability.py --duration-seconds 60` | Full local media/transcript/visual/note pipeline using synthetic media and synthetic timed subtitles | None | A 60-second task reaches `success`, writes note/transcript/visual/resource artifacts, and makes zero remote calls |
