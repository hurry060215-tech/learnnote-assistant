...
助手默认只显示对话和输入。能力与上下文保留在“回答选项”及回复的“来源与处理方式”，不占据起始页。

模型回复通过 POST `/api/assistant/execute/stream` 或 `/api/tasks/{id}/qa/stream` 接收真实 SSE 文本增量。等待状态不包含模型内部推理；停止、来源切换或失败会中止请求，不把半截回答保存为完整历史。本地说明和检索直接显示现成结果，不模拟模型打字。

左右侧栏支持拖动、键盘调宽和重置，宽度保存在本机。视频位于正文上方，可选择固定；打开助手不再改变播放器节点或播放时间。处理步骤与逐步记录可以折叠。

新建与扩展统一使用 `content_mode`：`subtitles` 只提取已有字幕，未取得就停止；`text` 优先字幕、缺少时转写并生成文字笔记；`visual` 使用视频和视觉模型；`auto` 仅用于旧配置兼容。字幕专用模式不会调用模型，也不会回退为页面文本总结。

旧的120秒检索窗口不代表视觉理解。现在根据字幕原始句段、停顿和长度上限组段，并保留相邻上下文；这些是检索片段，不被称为模型生成的语义章节。画面理解是否启用、选择何种模型，在新建界面单独说明。
...
后端完整回归 460 项通过，脚本和桌面回归继续通过。实际 Windows 0.2.3 打包程序通过全局帮助、显式 Skill、缺少上下文提示、功能目录、遮罩关闭、未保存保护、返回、置顶、历史回读和手机宽度检查；原工作台与工具流程也通过。模型调用使用模拟或缺配置分支验证，未把真实模型效果当作本轮通过结论。
