...
- 本次验收代码 commit：`f8d6250662f7afdf68bafcd5e107861eed4a6747`
...
另有隔离的 Playwright 浏览器验收：Windows / Edge，从学习计划设置和构造 Markdown 导入开始，完成全文阅读、课程筛选、生成并确认复习卡、键盘输入自我解释、查看原文出处、评分、导出备份、清空隔离学习库后从设置页恢复备份。恢复后 12 张卡片、1 条评分和 `Asia/Shanghai` 计划均保留。课程选择、自我解释、来源打开、评分、计划保存、备份导出/恢复均走键盘激活。
...
- #136：时区/DST/恢复自动回归通过；浏览器设置页保存并恢复 `Asia/Shanghai` 已通过键盘浏览器验收。首次自动时区建议和跨时区手工切换仍需验收。
...
因此四个 Issue 暂时继续开放。此包证明了隔离 API 数据路径和桌面/平板/手机浏览器学习路径可连续工作；Obsidian 双向同步、真实课程题目质量与首次时区自动建议仍是剩余条件。
