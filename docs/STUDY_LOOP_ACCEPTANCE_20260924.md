...
另有隔离的 Playwright 浏览器验收：Windows / Edge，首个 browser context 使用本机实际时区，设置页初始建议必须等于 `Intl.DateTimeFormat().resolvedOptions().timeZone`；随后手动选择与建议不同的计划时区。测试不修改 Windows 时区，也不会伪造浏览器 Intl 结果。从设置和构造 Markdown 导入开始，连续完成全文阅读、课程筛选、生成并确认复习卡、键盘输入自我解释、查看原文出处、评分、导出备份、清空隔离学习库后从设置页恢复备份。恢复后 12 张卡片、1 条评分和手选计划时区均保留。随后用另一个浏览器 context 请求不同的时区重开应用，计划仍保持手选值。课程选择、自我解释、来源打开、评分、计划保存、备份导出/恢复均走键盘激活。若 CI runner 忽略 Playwright 时区请求，验收仍检查 runner 自己的 Intl 时区与自动建议一致，并且保存值不同且重开后保留。
