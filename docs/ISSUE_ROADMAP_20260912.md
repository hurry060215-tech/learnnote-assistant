...
更新时间：2026-09-17
开放 issue 快照：31 个，来自仓库当前 GitHub open issues 列表；#133 已在 v0.2.13 发布验收后关闭。
正式基线：`v0.2.13`，`origin/main` 提交 `e93ac3d`（PR #189）。
本批收口台账：见 `docs/ISSUE_CLOSEOUT_0213.md`；0.2.12 收口记录保留为历史证据。
...
| B | #133 | 已完成 | v0.2.13 当前 tag 的 Windows/扩展/容器依赖、SBOM、构建来源、provenance、SHA256 与安装/升级门禁全部通过；Issue 已关闭。 | 非 bit-for-bit 的签名时间戳按 issue 非目标处理；Windows 签名凭据缺失已在 Release 记录。 |
...
- #55/#134：共享版本状态服务、更新中心入口、24 小时检查偏好、后台下载状态、取消、官方资产校验和任务/未保存内容保护已加入主线；安装版和便携版已在 v0.2.13 通过，但商店路径、失败回退和同标签 draft 恢复仍按各自 Issue 保留。
