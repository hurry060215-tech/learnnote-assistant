...
更新时间：2026-09-12
开放 issue 快照：33 个，来自仓库当前 GitHub open issues 列表。
基线：origin/main，提交 6ce935c。实施分支：codex/issue-roadmap-20260912。

这份台账对应 #52。它记录当前代码证据、剩余缺口和验收边界，不把已经存在的函数、测试或界面入口直接当作 issue 已完成。最近恢复的 issue 表单提交属于基线变更，不纳入本轮功能修复。
...
| B | #55 | 局部实现 | extension/manifest.json、scripts/package-extension.ps1、docs/RELEASING.md 已有 MV3 包和商店提交边界；desktop/main.py 已有正式版检查、校验下载、静默安装。 | 新工作台更新中心、Chrome/Edge 商店与受管理解压版流程、权限/审核/账号条件、真实安装路径和发布证据仍待验收。 |
...
| B | #134 | 局部实现 | scripts/publish-release.ps1、校验和、draft release 复用和 release workflow 已有发布基础；更新包现在要求官方路径与 SHA-256。 | 断网、损坏包、磁盘不足、文件占用、失败恢复、回退版本、健康检查后提交结果和安全重跑需真实验证。 |
...
| D | #146 | 局部实现 | backend/app/local_models.py、model_connections.py、assistant_skills.py 已有本地模型准备、连接状态和本地回退。 | 明确本地优先路线、离线就绪度、远程模型选择、能力/证据状态和失败时不伪装成模型结果。 |
...
| F | #153 | 计划中 | web/desk.css、product.css、experience.css 已有阅读壳、响应式和深浅主题。 | 390/768/1024/1440、200% 缩放、键盘、减少动效、状态和展开项统一；毛玻璃只用于外壳，正文保持实色高对比。 |
...
| F | #142 | 计划中 | knowledge/evidence 搜索和 citations 提供概念图谱所需的部分来源能力。 | 先完成结论引用体系；每条关系回到具体字幕/画面/页码，并提供无图列表替代视图。 |
