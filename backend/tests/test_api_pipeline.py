...
class LocalUploadValidationTests(unittest.TestCase):
    ...
    def test_health_reports_duration_probe_fallback(self) -> None:
        ...
        self.assertIn("llm_model_configured", payload)
        self.assertIsInstance(payload["llm_model_configured"], bool)
        ...
        deepseek_preset = next(item for item in payload["model_provider_presets"] if item["key"] == "deepseek")
        self.assertEqual(deepseek_preset["model"], "deepseek-flash")
        self.assertEqual(deepseek_preset["capabilities"], ["text", "vision"])
        kimi_preset = next(item for item in payload["model_provider_presets"] if item["key"] == "kimi")
        self.assertIn("vision", kimi_preset["capabilities"])
