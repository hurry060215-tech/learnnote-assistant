...
export function installProfile({ state, notice, startReview }) {
  ...
  async function openProfile() {
    ...
    if (study?.activity_today) {
      const activity = study.activity_today;
      const activityLine = document.createElement("p");
      activityLine.className = "muted";
      activityLine.textContent = "今日活动（按 " + (study.timezone || "计划时区") + "）：阅读 " + (activity.reading || 0) + " · 答题 " + (activity.answer || 0) + " · 自评 " + (activity.self_assessment || 0) + " · 复习 " + (activity.review || 0);
