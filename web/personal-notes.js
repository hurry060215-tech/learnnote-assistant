  ...
  async function attach({container, kind, id, apiUrl, fetchJson}) {
    ...
    const load=async()=>{
      ...
      for(const item of data.annotations||[]){
        ...
        const edit=document.createElement('button');edit.type='button';edit.textContent='编辑';edit.onclick=()=>{editing=item.id;editingAnchor=item.anchor&&typeof item.anchor==='object'?item.anchor:{};input.value=item.text;quote.textContent=item.quote;input.focus();};
    ...
    save.onclick=async()=>{if(!input.value.trim())return;save.disabled=true;try{const selected=quote.textContent.trim().slice(0,1000);let quoteHash='';if(selected&&globalThis.crypto?.subtle){const digest=await globalThis.crypto.subtle.digest('SHA-256',new TextEncoder().encode(selected));quoteHash=[...new Uint8Array(digest)].map(value=>value.toString(16).padStart(2,'0')).join('')}await fetchJson(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:input.value,quote:selected,id:editing,anchor:{...editingAnchor,source_revision:container.dataset.sourceRevision||editingAnchor.source_revision||'',selected_text:selected,quote_hash:quoteHash||editingAnchor.quote_hash||''}})});editing='';editingAnchor={};input.value='';drafts.delete(`${kind}:${id}`);quote.textContent='';status.textContent='批注已保存在本机。';await load()}catch(e){status.textContent=e.message}finally{save.disabled=false}};
