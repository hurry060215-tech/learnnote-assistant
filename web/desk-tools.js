...
export function installTools(ctx) {
  ...
  function exports() {
    ...
    const preview = async () => {
      ...
      try { const result = await api(`/api/tasks/${s.id}/exports/preview`, { method: "POST", body: JSON.stringify(collect()) }); $("unifiedExportPreview").srcdoc = result.html; status.textContent = result.warnings?.length ? `预览已生成：${result.warnings.join("、")}` : "预览已更新；HTML 可离线打开。"; } catch (error) { status.textContent = error.message; }
    };
